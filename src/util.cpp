...
const char * const BITCOIN_CONF_FILENAME = "luckycoinoro.conf";
const char * const BITCOIN_PID_FILENAME = "luckycoinorod.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "luckycoinorod -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LuckyCoinORO
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LuckyCoinORO
    // Mac: ~/Library/Application Support/LuckyCoinORO
    // Unix: ~/.luckycoinoro
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LuckyCoinORO";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/LuckyCoinORO";
#else
    // Unix
    return pathRet / ".luckycoinoro";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty luckycoinoro.conf if it does not excist
