...
LuckyCoinORO now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
Format: ```mngovernance prepare proposal-name url payment-count block-start luckycoinoro-address monthly-payment-luckycoinoro```
...
Format: ```mngovernance submit proposal-name url payment-count block-start luckycoinoro-address monthly-payment-luckycoinoro fee-tx```
...
When block ```1000000``` is reached you'll receive a payment for ```1200``` ORO to ```y6R9oN12KnB9zydzTLc3LikD9cCjjQzYG7```.
...
The following RPC commands are supported:
 ...
 - mngovernance "command"... ( "passphrase" )
  ...
  - vote               - Vote on a proposal by single masternode (using luckycoinoro.conf setup)
