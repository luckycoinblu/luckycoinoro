...
/**
 * Fees smaller than this (in Yngvarr) are considered zero fee (for transaction creation)
    ...
    for (auto pcoin : GetSpendableTXs()) {
        ...
        for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
            ...
            if (nCoinType == CoinType::ONLY_FULLY_MIXED) {
                ...
                found = pcoin->tx->vout[i].nValue == 108000*COIN;
    ...
    for (const auto& outpoint : setWalletUTXO) {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                if(fMasternodeMode && wtx.tx->vout[i].nValue == 108000*COIN) continue;
