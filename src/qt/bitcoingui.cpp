...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsMenuAction->setStatusTip(tr("Send coins to a LuckyCoinORO address"));
    sendCoinsMenuAction->setToolTip(sendCoinsMenuAction->statusTip());

    coinJoinCoinsMenuAction = new QAction("&CoinJoin", this);
    coinJoinCoinsMenuAction->setStatusTip(tr("Send %1 funds to a LuckyCoinORO address").arg("CoinJoin"));
    coinJoinCoinsMenuAction->setToolTip(coinJoinCoinsMenuAction->statusTip());

    receiveCoinsMenuAction = new QAction(tr("&Receive"), this);
    receiveCoinsMenuAction->setStatusTip(tr("Request payments (generates QR codes and luckycoinoro: URIs)"));
    ...
    aboutAction->setStatusTip(tr("Show information about LuckyCoinORO Core"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your LuckyCoinORO addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified LuckyCoinORO addresses"));
    ...
    openAction->setStatusTip(tr("Open a luckycoinoro: URI or payment request"));

    showHelpMessageAction = new QAction(tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible LuckyCoinORO command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (fNetworkActive) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to LuckyCoinORO network", "", count));
    } else {
...
void BitcoinGUI::updateWidth()
{
    ...
    // Use nButtonsVisible + 1 <- for the luckycoinoro logo
...
{
    QString strTitle = tr("LuckyCoinORO Core"); // default title
    ...
    // Append title to "LuckyCoinORO Core - "
    if (!msgType.isEmpty())
        strTitle += " - " + msgType;
