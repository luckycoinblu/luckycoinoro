...
*Setup instructions for a Gitian build of LuckyCoinORO Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the LuckyCoinORO
...
- [Building LuckyCoinORO Core](#building-luckycoinoro-core)
...
Clone the git repositories for LuckyCoinORO Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/luckycoinoropay/luckycoinoro
...
in the LuckyCoinORO Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building LuckyCoinORO Core
----------------

To build LuckyCoinORO Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the LuckyCoinORO Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/luckycoinoro/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/luckycoinoropay/luckycoinoro
...
For example:
```bash
URL=https://github.com/crowning-/luckycoinoro.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit luckycoinoro=${COMMIT} --url luckycoinoro=${URL} ../luckycoinoro/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit luckycoinoro=${COMMIT} --url luckycoinoro=${URL} ../luckycoinoro/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit luckycoinoro=${COMMIT} --url luckycoinoro=${URL} ../luckycoinoro/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the luckycoinoro git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../luckycoinoro/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/luckycoinoro
SIGPATH=/some/root/path/luckycoinoro-detached-sigs

./bin/gbuild --url luckycoinoro=${BTCPATH},signature=${SIGPATH} ../luckycoinoro/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/luckycoinoro-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/luckycoinoro-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/luckycoinoro-osx-build.assert
```
...
[luckycoinoro/gitian.sigs](https://github.com/luckycoinoropay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
