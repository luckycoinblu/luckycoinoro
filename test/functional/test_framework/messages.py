...
def luckycoinorohash(s):
    return luckycoinoro_hash.getPoWHash(s)
...
# Objects that map to luckycoinorod objects, which can be serialized/deserialized
...
class CBlockHeader():
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(luckycoinorohash(r))
            self.hash = encode(luckycoinorohash(r)[::-1], 'hex_codec').decode('ascii')
...
class msg_headers():
    ...
    def deserialize(self, f):
        # comment in luckycoinorod indicates these should be deserialized as blocks
