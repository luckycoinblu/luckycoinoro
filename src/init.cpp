...
public:
    ...
    // LuckyCoinORO Specific WalletInitInterface InitCoinJoinSettings
 ...
 * between luckycoinorod, and luckycoinoro-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    RenameThread("luckycoinoro-shutoff");
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-sporkaddr=<luckycoinoroaddress>", "Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.", false, OptionsCategory::DEBUG_TEST);
    ...
    gArgs.AddArg("-platform-user=<user>", "Set the username for the \"platform user\", a restricted user intended to be used by LuckyCoinORO Platform, to the specified username.", false, OptionsCategory::MASTERNODE);
...
std::string LicenseInfo()
{
    ...
    const std::string URL_WEBSITE = "<https://oroworld.info>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("luckycoinoro-loadblk");
...
/** Sanity checks
 *  Ensure that LuckyCoinORO Core is running in a usable environment with all
...
{
    // Make sure only a single LuckyCoinORO Core process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if LuckyCoinORO Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if LuckyCoinORO Core is started while in a temporary directory.\n",
    ...
    // ********************************************************* Step 10c: schedule LuckyCoinORO-specific tasks
