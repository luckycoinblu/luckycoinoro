...
export BUILDER_IMAGE_NAME="luckycoinoro-builder-$BUILD_TARGET-$JOB_NUMBER"
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export USE_SHELL="/bin/luckycoinoro"
  export PYZMQ=true
  export RUN_UNITTESTS=true
  ...
  export CPPFLAGS="-DDEBUG_LOCKORDER -DENABLE_ORO_DEBUG -DARENA_DEBUG"
  export PYZMQ=true
  export RUN_UNITTESTS=true
  export RUN_INTEGRATIONTESTS=true
