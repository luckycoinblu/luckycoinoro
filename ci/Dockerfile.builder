...
# luckycoinoro_hash
RUN git clone https://github.com/luckycoinoropay/luckycoinoro_hash
RUN cd luckycoinoro_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} luckycoinoro
RUN useradd -u ${USER_ID} -g luckycoinoro -s /bin/bash -m -d /luckycoinoro luckycoinoro
...
RUN mkdir /luckycoinoro-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /luckycoinoro-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /luckycoinoro-src
